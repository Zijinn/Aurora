...
export function SubmittedPage(props: {
  ...
  // desktop WKWebView shell. At most one card shows the form at a time.
            ...
            return (
              <Card key={paper.id} id={paper.id} onReorder={reorder}>
                <div className="wb-card-top">
                  <div>
                    <div className="wb-card-eyebrow">
                      {displayID("submitted", index)} · {t("eyebrowSubmission")}
                    </div>
                ...
                <div className="wb-card-line">
                  <span className="wb-muted">{t("currentJournal")}</span>
                  <strong>
                    <InlineText
                ...
                <div className="wb-card-line">
                  ...
                  <strong>
                    <InlineText
                      ...
                      placeholder={t("fillPlaceholder")}
                      onCommit={(value) => props.onUpdate(paper.id, { submission_date: value })}
                    />
                  </strong>
                </div>
                <div className="wb-card-line">
                  ...
                  <strong>
                    <InlineText
                      ...
                      placeholder={t("fillPlaceholder")}
                      onCommit={(value) => props.onUpdate(paper.id, { manuscript_id: value })}
                    />
                  </strong>
                </div>
                <div className="wb-card-line">
                  <span className="wb-muted">{t("submissionCountLabel")}</span>
                  <strong>
                    <InlineText
                  ...
                  {historyDraft?.paperID === paper.id && (
                    <div className="wb-history-add">
                      ...
                      <input
                        ...
                        onChange={(e) => setHistoryDraft({ ...historyDraft, date: e.target.value })}
                        onKeyDown={(e) => {
                          if (e.key === "Enter") {
                            e.preventDefault()
                            commitHistoryAdd()
                      ...
                      />
                      <button type="button" className="wb-btn" onClick={commitHistoryAdd}>
                        {t("add")}
                      </button>
                      <button
                        type="button"
                        ...
                        >
                          ...
                          <div className="wb-timeline-content">
                            ...
                            <select
                              ...
                              value={record.status}
                              onChange={(e) =>
                                updateHistory(paper, historyIndex, { status: e.target.value })
                              }
                            >
                              {SUBMISSION_STATUS_OPTIONS.map((option) => (
                                <option key={option.value} value={option.value}>
                            ...
                            <span className={`wb-badge ${statusBadgeClass(record.status)}`}>
                              {t(
                                SUBMISSION_STATUS_OPTIONS.find((o) => o.value === record.status)
                                  ?.key ?? "statusSubmitted",
                              )}
                            </span>
                            <button
