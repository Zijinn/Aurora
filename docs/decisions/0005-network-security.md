...
- Remote access over the public internet requires TLS through a documented reverse proxy or the optional built-in certificate/key configuration (`CAIRN_TLS_CERT_PATH` and `CAIRN_TLS_KEY_PATH`). Reverse proxy peers are trusted only when their exact IP or CIDR is listed in `CAIRN_TRUSTED_PROXIES`; classification uses the immediate TCP peer and never `X-Forwarded-For`. Proxy ingress may use an external `Host`, but protected APIs always require a paired device token.
