...
- The Windows artifact is a per-user x64 NSIS installer. Install on a clean Windows 10/11 runner, confirm WebView2 bootstrap behavior, launch Cairn from the installed path, and verify that uninstall leaves user data untouched. The installer must contain `web/dist` beside `Cairn.exe`.
- If notarized, verify with `spctl --assess --type execute` and `xcrun stapler validate`.
- Publish the `.sha256` files beside the installers and retain the license reports with the release.

## Recovery and rollback

- Keep the previous installer and its checksum available until the new version passes clean-install and migration checks.
